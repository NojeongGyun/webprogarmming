...
<b>그림자(텍스트/박스)</b> -
그림자를 텍스트나 박스에 넣을 수 있는데, 박스는  [box-shadow: 오른쪽, 아래, 흐림 정도, 퍼짐 정도, 색깔] 이고 텍스트는 [text-shadow: 오른쪽, 아래, 흐림 정도, 색깔]으로 사용됩니다.

________________________________________________________chapter 5 _________________________________________________________________
 
- <mark>display</mark> -
display는 CSS에서 요소가 화면에 어떻게 배치되고 보여질지 결정하는 가장 중요한 프로퍼티입니다. display는 종류가 블록박스, 인라인 박스, 인라인 블록 박스가 있습니다.

<b>display Block Box</b> -
블록 박스는 웹페이지를 구조화 할떄 사용되니다. 다른 블록박스가 나오면 새로운 라인에서 시작하기에 콘텐츠의 구조를 나누는데 용이합니다. 
블록 박스의 외관의 구조 너비와 높이는 width, height로 조정가능하며, 박스안은 패딩, 마진, 테두리를 사용해서 조정가능합니다. 
사용방법은 블록 요소 태그(span, div, p, h1~h6, ul, ol, li)와 같이 사용합니다.
ex)  
   - span 블록태그에 블럭박스를 넣고, 블럭박스에 너비를 100px, 높이를 60px로 조정하여 보여줌 -
      <code> span {
             display : block;
             width : 100px;
             height : 60px;
...
<b>display Inline Box</b> -
인라인 박스는 텍스트나 이미지처럼 콘텐츠의 흐름을 방해하지 않고 줄바꿈 없이 박스형태로 가로로 나란히 배치됩니다. 인라인 박스는 블록 박스와 다르게 각 박스의 외관/내관 크기를 조절하지 못합니다.
ex) 
   - 부모태그인 div 안의 자손인 div를 모두 같은 박스로 넣어서 보여줌 -
       <code> div div {
              display : inline;
              } </code>
 
<b>display Inline Block Box</b> -
인라인 블록 박스는 블록 박스의 외관/내관 크기 조정와 구조 조정이 가능한 부분과 인라인 박스의 줄바꿈 없이 박스 형태로 가로로 나란히 배치 되는 점을 합친 display를 말합니다.
ex)
    - 부모태그인 div 안의 자손인 div를 모두 같은 박스로 넣어서 보여주되, 스타일을 적용시킨다. -
       <code> div div {
              display : inline-block; 
              border : 2px dotted orangered ; /* 테두리를 2pt, 점선, 테두리를 주황색으로 적용 */
              background : powderblue;  /* 개별 박스의 배경색을 하늘색으로 변경 */
              margin : 10px;  /* 여백을 10pt 적용 */
              width : 60px; height : 80px;  /* 박스 너비, 높이를 60px, 80px 적용 */
              }</code>

 
- <mark>배치</mark> -
배치는 해당 블록 요소를 원하는 위치에 이동시키는 걸 말하며, 정적 배치,  상대 배치, 절대 배치, 고정 배치, 유동 배치가 있습니다.
...
ex) 
     - ID가 down인 태그에 배경색 초록을 적용시키고, 마우스를 갖다 대면 오른쪽으로 20px, 아래로 20px 옮김  -
       <code>#down:hover {
              ...
              top : 20px; /* 기준점(위쪽)과의 거리를 나타내므로 아래로 20px 이동 */
              background : green;
              } </code>
